  ...
  .define(
    ...
    function(exports) {
      ...
      self.loading = function(elem) {
        elem.append(
          (this.elemLoad = $(
            '<i class="layui-anim layui-anim-rotate layui-anim-loop layui-icon layui-icon-loading eonadmin-loading"></i>'
          ))
        )
      }
      ...
      self.tab = {
        ...
        minLeft: null,
        maxLeft: null,
        wrap: '.eonadmin-tabs-wrap',
        menu: '.eonadmin-tabs-menu',
        next: '.eonadmin-tabs-next',
        prev: '.eonadmin-tabs-prev',
        step: 200,
        init: function() {
          var tab = this
          var btnCls = tab.wrap + ' .eonadmin-tabs-btn'

          layui.dropdown.render({
            elem: '.eonadmin-tabs-down',
            click: function(name) {
          ...
          $(document).on('click', btnCls, function(e) {
            ...
            if ($(e.target).hasClass('eonadmin-tabs-close')) {
              tab.del(url)
            } else {
          ...
          $('.eonadmin-tabs-hidden').addClass('layui-show')
          this.isInit = true
        },
        has: function(url) {
          var exists = false
        ...
        delAll: function(type) {
          var tab = this
          var menuBtnClas = tab.menu + ' .eonadmin-tabs-btn'
        ...
        delOther: function() {
          var tab = this
          var menuBtnClas = tab.menu + ' .eonadmin-tabs-btn'
          $(menuBtnClas + '.eonadmin-tabs-active')
            .siblings()
            .each(function() {
              var url = $(this).attr('lay-url')
              tab.del(url)
        ...
        del: function(url, backgroundDel) {
          ...
          var lay = '[lay-url="' + url + '"]'
          var thisBody = $(
            '#' + conf.containerBody + ' > .eonadmin-tabs-item' + lay
          )
          ...
          if (backgroundDel === undefined) {
            if (thisMenu.hasClass('eonadmin-tabs-active')) {
        ...
        clear: function() {
          this.data = []
          this.isInit = false
          $(document).off('click', this.wrap + ' .eonadmin-tabs-btn')
        },
        change: function(route, callback) {
          ...
          var activeCls = 'eonadmin-tabs-active'
          ...
          if (existsTab) {
            ...
            self.loadHtml(fileurl, function(res) {
              var htmlElem = $(
                "<div><div class='eonadmin-tabs-item' lay-url='" +
                  fileurl +
                  "'>" +
                  res.html +
                  '</div></div>'
        ...
        if (tab.change(data.fileurl) === false) {
          ...
          if (data) {
            ...
            thisMenu
              .addClass('eonadmin-tabs-active')
              .siblings()
              .removeClass('eonadmin-tabs-active')
            this.minLeft = this.minLeft || parseInt(menu.css('left'))
            ...
            $('#' + conf.containerBody + ' > .eonadmin-tabs-item' + layUrl)
              .show()
              .siblings()
              .hide()
            $(document).scrollTop(-100)
      ...
      self.fillHtml = function(url, htmlElem, modeName) {
        ...
        //重新对面包屑进行渲染
        layui.element.render('breadcrumb', 'eonadmin-breadcrumb')
      ...
      self.renderLayout = function(callback, url) {
        ...
        self.render(url, function(res) {
          ...
          if (conf.viewTabs == true) {
            self.containerBody.addClass('eonadmin-tabs-body')
          }
