...
(function ($) {
    ...
    WebUploader.Uploader.register({
            ...
            (new WebUploader.Uploader()).md5File(file, 0, 10*1024*1024).progress(function(percentage){
                ...
                $.ajax({
                    ...
                    if(data.code==0) {
                        ...
                        if(!(data.data.chunk>=0)){
                            file.statusText = '无法获取当前文件块';
                            task.reject();
                            return
                        }
                        chunkMap.put(file_ruid,{fileId:fileMd5,fileName:fileName,fileSize:fileSize,fileMd5:fileMd5,chunkCurr:data.data.chunk});
                        task.resolve();
        ...
        , beforeSend: function(block){
           // var that=this.owner.controller;
            ...
            if(block.chunk<chunkCurr-1){
                //console.log(block)
                task.reject();
            }else {
                console.log("第"+(block.chunk+1)+"块开始上传")
                task.resolve();
            }
    ...
    var Controller = function (input, option) {
        ...
        this.defaultAccept = {
            title:'支持的文件类型',
            extensions: 'jpg,jpeg,png,gif,bmp,doc,docx,pdf,xls,xlsx,ppt,pptx,zip,rar,xml',
        ...
        this.option = $.extend(false, {
            ...
            'checkUrl': getBaseUrl()+'/file/chunk/check',     // string
            'uploadUrl': getBaseUrl()+'/file/chunk/upload',   // string
    ...
    var _setup = function (input) {
        ...
        _uploader.on( 'uploadBeforeSend', function( block, data ) {
            data.appNo = 'vcf_test';
            data.fileId = chunkMap.get(block.file.id).fileId;
        ...
        _uploader.on('uploadSuccess', function(file ,resp) {
            if (resp != null) {
                if (resp.code==0) {
                    ...
                    var item=$('#item_'+file.id);
                    item.find('.upload_state').text('上传出错:'+resp.msg);
                    item.find('.re_upload').show();
                    console.log('上传error:'+resp.msg)
                }
            }
        });
